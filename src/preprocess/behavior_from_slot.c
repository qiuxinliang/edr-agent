...
/* These are uint64_t mask values, not C enum members. MSVC's C enum
 * representation narrows bits 32+ to zero and sign-extends bit 31 even
 * when the initializer uses 1ull. Keep the constants wide before any OR. */
#define EDR_ETW_TRUNC_IMAGE (UINT64_C(1) << 0)
#define EDR_ETW_TRUNC_IMAGE_RAW (UINT64_C(1) << 1)
#define EDR_ETW_TRUNC_IMAGE_CANONICAL (UINT64_C(1) << 2)
#define EDR_ETW_TRUNC_CMDLINE (UINT64_C(1) << 3)
#define EDR_ETW_TRUNC_USERNAME (UINT64_C(1) << 4)
#define EDR_ETW_TRUNC_DOMAIN (UINT64_C(1) << 5)
#define EDR_ETW_TRUNC_USER_SID (UINT64_C(1) << 6)
#define EDR_ETW_TRUNC_LOGON_ID (UINT64_C(1) << 7)
#define EDR_ETW_TRUNC_CREATOR_USERNAME (UINT64_C(1) << 8)
#define EDR_ETW_TRUNC_CREATOR_DOMAIN (UINT64_C(1) << 9)
#define EDR_ETW_TRUNC_CREATOR_SID (UINT64_C(1) << 10)
#define EDR_ETW_TRUNC_CREATOR_LOGON_ID (UINT64_C(1) << 11)
#define EDR_ETW_TRUNC_PARENT_IMAGE (UINT64_C(1) << 12)
#define EDR_ETW_TRUNC_PARENT_CMDLINE (UINT64_C(1) << 13)
#define EDR_ETW_TRUNC_CURRENT_DIRECTORY (UINT64_C(1) << 14)
#define EDR_ETW_TRUNC_INTEGRITY (UINT64_C(1) << 15)
#define EDR_ETW_TRUNC_TOKEN_ELEVATION (UINT64_C(1) << 16)
#define EDR_ETW_TRUNC_PROCESS_CREATION_TIME (UINT64_C(1) << 17)
#define EDR_ETW_TRUNC_PROCESS_GENERATION_SOURCE (UINT64_C(1) << 18)
#define EDR_ETW_TRUNC_SOURCE_COMPLETENESS (UINT64_C(1) << 19)
#define EDR_ETW_TRUNC_FILE_PATH (UINT64_C(1) << 20)
#define EDR_ETW_TRUNC_OLD_FILE_PATH (UINT64_C(1) << 21)
#define EDR_ETW_TRUNC_EXE_HASH (UINT64_C(1) << 22)
#define EDR_ETW_TRUNC_NET_PROTO (UINT64_C(1) << 23)
#define EDR_ETW_TRUNC_REG_OP (UINT64_C(1) << 24)
#define EDR_ETW_TRUNC_IMAGE_NAMESPACE (UINT64_C(1) << 25)
#define EDR_ETW_TRUNC_IMAGE_RESOLUTION_STATUS (UINT64_C(1) << 26)
#define EDR_ETW_TRUNC_IMAGE_RESOLUTION_SOURCE (UINT64_C(1) << 27)
#define EDR_ETW_TRUNC_REG_KEY_PATH (UINT64_C(1) << 28)
#define EDR_ETW_TRUNC_REG_VALUE_NAME (UINT64_C(1) << 29)
#define EDR_ETW_TRUNC_REG_OLD_VALUE_DATA (UINT64_C(1) << 30)
#define EDR_ETW_TRUNC_DNS_QUERY (UINT64_C(1) << 31)
#define EDR_ETW_TRUNC_SOURCE_TRUNCATED_FIELDS (UINT64_C(1) << 32)
#define EDR_ETW_TRUNC_COMMAND_LINE_ORIGIN (UINT64_C(1) << 33)

_Static_assert(EDR_ETW_TRUNC_DNS_QUERY == UINT64_C(0x80000000),
               "DNS truncation bit must not sign-extend");
_Static_assert(EDR_ETW_TRUNC_SOURCE_TRUNCATED_FIELDS == UINT64_C(0x100000000),
               "truncation-list overflow bit must survive above bit 31");
_Static_assert(EDR_ETW_TRUNC_COMMAND_LINE_ORIGIN == UINT64_C(0x200000000),
               "command origin truncation bit must survive above bit 31");
