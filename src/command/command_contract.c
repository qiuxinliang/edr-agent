...
static const CommandFieldRule k_agent_update_rules[] = {
    ...
    RULE("min_current_version", FIELD_STRING, 0, 0, 0, 64, 0),
...
static int validate_semantics(EdrCommandKind kind, const cJSON *root,
  ...
  if (kind == EDR_COMMAND_KIND_AGENT_UPDATE) {
    ...
    if (mode && (!mode->valuestring || (strcmp(mode->valuestring, "auto") &&
      ...
      return contract_fail(reason, reason_cap, "agent_update deployment_mode is invalid");
    }
    if (!!manifest_url != !!manifest_sha) {
      return contract_fail(reason, reason_cap, "runtime manifest URL and SHA256 must be supplied together");
    }
