...
/* ── Host Actions ──
 * 主机隔离/恢复已统一到 command_stub.c 的 do_isolate / do_restore_host(默认真实 OS
 * 网络 enforcement + 自动放行管理通道)。此处旧的 stamp-only 实现(edr_response_isolate /
 * edr_response_restore_host / edr_response_isolate_auto_from_shellcode)未接入任何派发表,
 * 且会谎报隔离成功,已删除;shellcode 自动隔离走 command_stub.c 的 do_isolate("auto-shellcode")。 */
