#if !defined(_WIN32)
#error "webshell_detector_win.c is Windows-only"
#endif

#include "edr/webshell_detector.h"

#include "edr/ave_sdk.h"
#include "edr/config.h"
#include "edr/event_bus.h"
#include "edr/grpc_client.h"
#include "edr/ingest_http.h"
#include "edr/types.h"
#include "edr/webshell_forensic.h"
#include "edr/edr_log.h"
#include "edr/webshell_semantic.h"

#include <direct.h>
#include <io.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h>
#include <windows.h>

#ifdef EDR_HAVE_YARA
#include <yara.h>
#endif

#define WEBSHELL_MAX_ROOTS 64
#define WEBSHELL_MAX_WATCHES 64

typedef struct {
  char root[MAX_PATH];
  char service_name[32];
  uint16_t port;
} WebRoot;

typedef struct {
  char rule_name[128];
  float confidence;
  float ast_score;
  float token_score;
  int matched;
} WebshellRuleMatch;

typedef struct {
  HANDLE dir_handle;
  HANDLE thread;
  char root_path[MAX_PATH];
  int active;
} WatchEntry;

static const EdrConfig *s_cfg;
static EdrEventBus *s_bus;
static int s_started;
static volatile LONG s_stop;
static WebRoot s_roots[WEBSHELL_MAX_ROOTS];
static size_t s_root_count;
static WatchEntry s_watches[WEBSHELL_MAX_WATCHES];
static size_t s_watch_count;
static uint64_t s_budget_window_ns;
static uint64_t s_budget_bytes;
static uint64_t s_budget_drops;

#ifdef EDR_HAVE_YARA
static YR_RULES *s_yara_rules;
static int s_yara_inited;
#endif

static const char *kExtAllow[] = {
    ".php", ".php3", ".php4", ".php5", ".php7", ".phtml", ".phar", ".asp", ".aspx", ".cer", ".asa", ".jsp", ".jspx",
    ".jspf", ".shtml", ".stm", ".py",  ".pl",   ".rb",   ".cgi",  ".htaccess", ".user.ini", ".exe", ".dll", ".so",
    NULL};

static uint64_t now_ns(void) {
  FILETIME ft;
  GetSystemTimePreciseAsFileTime(&ft);
  ULARGE_INTEGER u;
  u.LowPart = ft.dwLowDateTime;
  u.HighPart = ft.dwHighDateTime;
  const uint64_t epoch_100ns = 116444736000000000ULL;
  if (u.QuadPart < epoch_100ns) {
    return 0;
  }
  return (u.QuadPart - epoch_100ns) * 100ULL;
}

static int webshell_scan_budget_allow(uint64_t bytes) {
  if (!s_cfg || s_cfg->resource_limit.webshell_scan_mb_per_min == 0u) {
    return 1;
  }
  uint64_t now = now_ns();
  if (s_budget_window_ns == 0u || now < s_budget_window_ns ||
      now - s_budget_window_ns >= 60000000000ULL) {
    s_budget_window_ns = now;
    s_budget_bytes = 0u;
  }
  uint64_t cap = (uint64_t)s_cfg->resource_limit.webshell_scan_mb_per_min * 1024ULL * 1024ULL;
  if (bytes > cap || s_budget_bytes + bytes > cap) {
    s_budget_drops++;
    return 0;
  }
  s_budget_bytes += bytes;
  return 1;
}

static int contains_ci(const char *hay, const char *needle) {
  if (!hay || !needle || !needle[0]) {
    return 0;
  }
  size_t nlen = strlen(needle);
  size_t hlen = strlen(hay);
  if (nlen > hlen) {
    return 0;
  }
  for (size_t i = 0; i + nlen <= hlen; i++) {
    if (_strnicmp(hay + i, needle, nlen) == 0) {
      return 1;
    }
  }
  return 0;
}

static int ext_allowed(const char *path) {
  const char *dot = strrchr(path, '.');
  if (!dot) {
    return 0;
  }
  for (int i = 0; kExtAllow[i]; i++) {
    if (_stricmp(dot, kExtAllow[i]) == 0) {
      return 1;
    }
  }
  return 0;
}

static int in_whitelist(const char *path) {
  return contains_ci(path, "\\wp-includes\\") || contains_ci(path, "\\wp-admin\\") ||
         contains_ci(path, "\\thinkphp\\library\\") || contains_ci(path, "\\vendor\\laravel\\");
}

static int pre_filter(const char *path) {
  struct _stat64 st;
  if (_stat64(path, &st) != 0 || !(st.st_mode & _S_IFREG)) {
    return 0;
  }
  if (!ext_allowed(path) || in_whitelist(path)) {
    return 0;
  }
  uint64_t max_bytes = (uint64_t)s_cfg->webshell_detector.max_file_size_mb * 1024ULL * 1024ULL;
  if ((uint64_t)st.st_size == 0u || (uint64_t)st.st_size > max_bytes) {
    return 0;
  }
  if (!webshell_scan_budget_allow((uint64_t)st.st_size)) {
    return 0;
  }
  __int64 sz1 = st.st_size;
  Sleep(200u);
  if (_stat64(path, &st) != 0 || !(st.st_mode & _S_IFREG)) {
    return 0;
  }
  if (st.st_size != sz1) {
    Sleep(s_cfg->webshell_detector.defer_retry_ms);
    if (_stat64(path, &st) != 0 || !(st.st_mode & _S_IFREG)) {
      return 0;
    }
  }
  return 1;
}

static void infer_service(const char *root, char *svc, size_t svc_cap, uint16_t *port) {
  if (contains_ci(root, "inetpub")) {
    snprintf(svc, svc_cap, "%s", "IIS");
    *port = 80u;
  } else if (contains_ci(root, "nginx")) {
    snprintf(svc, svc_cap, "%s", "Nginx");
    *port = 80u;
  } else if (contains_ci(root, "apache") || contains_ci(root, "xampp")) {
    snprintf(svc, svc_cap, "%s", "Apache");
    *port = 80u;
  } else if (contains_ci(root, "tomcat")) {
    snprintf(svc, svc_cap, "%s", "Tomcat");
    *port = 8080u;
  } else {
    snprintf(svc, svc_cap, "%s", "WebService");
    *port = 80u;
  }
}

static void add_root(const char *path) {
  if (!path || !path[0] || s_root_count >= WEBSHELL_MAX_ROOTS) {
    return;
  }
  DWORD attr = GetFileAttributesA(path);
  if (attr == INVALID_FILE_ATTRIBUTES || !(attr & FILE_ATTRIBUTE_DIRECTORY)) {
    return;
  }
  for (size_t i = 0; i < s_root_count; i++) {
    if (_stricmp(s_roots[i].root, path) == 0) {
      return;
    }
  }
  WebRoot *r = &s_roots[s_root_count++];
  memset(r, 0, sizeof(*r));
  snprintf(r->root, sizeof(r->root), "%s", path);
  infer_service(path, r->service_name, sizeof(r->service_name), &r->port);
}

static void discover_web_roots(void) {
  s_root_count = 0;
  add_root("C:\\inetpub\\wwwroot");
  add_root("C:\\xampp\\htdocs");
  add_root("C:\\phpstudy_pro\\WWW");
  add_root("C:\\nginx\\html");
  add_root("C:\\tomcat\\webapps");
  const char *extra = getenv("EDR_WEBSHELL_ROOTS");
  if (!extra || !extra[0]) {
    return;
  }
  char *dup = _strdup(extra);
  if (!dup) {
    return;
  }
  char *ctx = NULL;
  for (char *tok = strtok_s(dup, ",;", &ctx); tok; tok = strtok_s(NULL, ",;", &ctx)) {
    while (*tok == ' ' || *tok == '\t') {
      tok++;
    }
    if (*tok) {
      add_root(tok);
    }
  }
  free(dup);
}

static const WebRoot *find_root_for_path(const char *path) {
  const WebRoot *best = NULL;
  size_t best_len = 0;
  for (size_t i = 0; i < s_root_count; i++) {
    size_t n = strlen(s_roots[i].root);
    if (n > best_len && _strnicmp(path, s_roots[i].root, n) == 0) {
      best = &s_roots[i];
      best_len = n;
    }
  }
  return best;
}

static void infer_web_url(const WebRoot *root, const char *file_path, char *out, size_t cap) {
  char host[128] = "localhost";
  DWORD n = (DWORD)sizeof(host);
  GetComputerNameA(host, &n);
  const char *rel = file_path + strlen(root->root);
  if (*rel == '\\' || *rel == '/') {
    rel++;
  }
  char rel_norm[MAX_PATH];
  snprintf(rel_norm, sizeof(rel_norm), "/%s", rel);
  for (char *p = rel_norm; *p; p++) {
    if (*p == '\\') {
      *p = '/';
    }
  }
  if (root->port == 80u) {
    snprintf(out, cap, "http://%s%s", host, rel_norm);
  } else if (root->port == 443u) {
    snprintf(out, cap, "https://%s%s", host, rel_norm);
  } else {
    snprintf(out, cap, "http://%s:%u%s", host, (unsigned)root->port, rel_norm);
  }
}

static float confidence_for_rule(const char *rule) {
  if (!rule || !rule[0]) {
    return 0.0f;
  }
  if (_stricmp(rule, "PHP_Webshell_ChinaChopper") == 0) {
    return 1.0f;
  }
  if (_stricmp(rule, "PHP_Webshell_OneLiners") == 0 || _stricmp(rule, "JSP_Webshell_ClassLoader") == 0) {
    return 0.95f;
  }
  if (_stricmp(rule, "JSP_Webshell_Runtime_Exec") == 0 || _stricmp(rule, "ASPX_Webshell_Eval") == 0) {
    return 0.90f;
  }
  if (_stricmp(rule, "ASPX_Webshell_CommandExec") == 0) {
    return 0.85f;
  }
  if (_stricmp(rule, "PHP_Webshell_FunctionObfuscation") == 0) {
    return 0.80f;
  }
  if (_stricmp(rule, "PHP_Webshell_SystemCommand") == 0) {
    return 0.75f;
  }
  if (_stricmp(rule, "PHP_Webshell_FilesystemWrite") == 0) {
    return 0.70f;
  }
  if (_stricmp(rule, "Generic_Webshell_Encoding_Layering") == 0) {
    return 0.65f;
  }
  return 0.60f;
}

#ifdef EDR_HAVE_YARA
static int is_rule_file(const char *name) {
  const char *dot = strrchr(name, '.');
  if (!dot) {
    return 0;
  }
  return (_stricmp(dot, ".yar") == 0 || _stricmp(dot, ".yara") == 0) ? 1 : 0;
}

static int yara_compile_cb(int level, const char *file_name, int line_number, const YR_RULE *rule, const char *msg,
                           void *user_data) {
  (void)level;
  (void)rule;
  (void)user_data;
  EDR_LOGE("[webshell_detector] yara compile error file=%s line=%d msg=%s\n", file_name ? file_name : "-", line_number,
          msg ? msg : "-");
  return 0;
}

static int yara_scan_cb(int message, void *message_data, void *user_data) {
  WebshellRuleMatch *m = (WebshellRuleMatch *)user_data;
  if (message == CALLBACK_MSG_RULE_MATCHING && m && !m->matched) {
    const YR_RULE *r = (const YR_RULE *)message_data;
    if (r) {
      snprintf(m->rule_name, sizeof(m->rule_name), "%s", r->identifier);
      m->confidence = confidence_for_rule(r->identifier);
      m->matched = 1;
      return CALLBACK_ABORT;
    }
  }
  return CALLBACK_CONTINUE;
}

static void unload_yara(void) {
  if (s_yara_rules) {
    yr_rules_destroy(s_yara_rules);
    s_yara_rules = NULL;
  }
  if (s_yara_inited) {
    yr_finalize();
    s_yara_inited = 0;
  }
}

static int load_yara(const char *dir) {
  if (!dir || !dir[0]) {
    return 0;
  }
  if (yr_initialize() != ERROR_SUCCESS) {
    return -1;
  }
  s_yara_inited = 1;
  YR_COMPILER *c = NULL;
  if (yr_compiler_create(&c) != ERROR_SUCCESS || !c) {
    unload_yara();
    return -1;
  }
  yr_compiler_set_callback(c, yara_compile_cb, NULL);

  char pattern[MAX_PATH];
  snprintf(pattern, sizeof(pattern), "%s\\*.*", dir);
  WIN32_FIND_DATAA fd;
  HANDLE h = FindFirstFileA(pattern, &fd);
  int loaded = 0;
  if (h != INVALID_HANDLE_VALUE) {
    do {
      if ((fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0u || !is_rule_file(fd.cFileName)) {
        continue;
      }
      char full[MAX_PATH];
      snprintf(full, sizeof(full), "%s\\%s", dir, fd.cFileName);
      FILE *fp = fopen(full, "rb");
      if (!fp) {
        continue;
      }
      int nerr = yr_compiler_add_file(c, fp, NULL, full);
      fclose(fp);
      if (nerr == 0) {
        loaded++;
      }
    } while (FindNextFileA(h, &fd));
    FindClose(h);
  }

  if (loaded <= 0) {
    yr_compiler_destroy(c);
    unload_yara();
    return 0;
  }
  if (yr_compiler_get_rules(c, &s_yara_rules) != ERROR_SUCCESS || !s_yara_rules) {
    yr_compiler_destroy(c);
    unload_yara();
    return -1;
  }
  yr_compiler_destroy(c);
  EDR_LOGV("[webshell_detector] yara rules loaded=%d from %s\n", loaded, dir);
  return loaded;
}
#endif

static int read_file_text(const char *path, char **out_buf) {
  *out_buf = NULL;
  FILE *f = fopen(path, "rb");
  if (!f) {
    return -1;
  }
  if (fseek(f, 0, SEEK_END) != 0) {
    fclose(f);
    return -1;
  }
  long sz = ftell(f);
  if (sz <= 0 || sz > (long)(10 * 1024 * 1024)) {
    fclose(f);
    return -1;
  }
  if (fseek(f, 0, SEEK_SET) != 0) {
    fclose(f);
    return -1;
  }
  char *buf = (char *)malloc((size_t)sz + 1u);
  if (!buf) {
    fclose(f);
    return -1;
  }
  size_t n = fread(buf, 1, (size_t)sz, f);
  fclose(f);
  buf[n] = '\0';
  *out_buf = buf;
  return 0;
}

static int fallback_match(const char *text, WebshellRuleMatch *m) {
  if (!text || !m) {
    return 0;
  }
  {
    EdrWebshellSemanticResult sem;
    if (edr_webshell_semantic_match_text(text, &sem)) {
      snprintf(m->rule_name, sizeof(m->rule_name), "%s", sem.rule_name);
      m->confidence = sem.confidence;
      m->ast_score = sem.ast_score;
      m->token_score = sem.token_score;
      m->matched = 1;
      return 1;
    }
  }
  if ((contains_ci(text, "eval(") && contains_ci(text, "$_POST")) ||
      contains_ci(text, "eval(base64_decode($_POST)")) {
    snprintf(m->rule_name, sizeof(m->rule_name), "%s", "PHP_Webshell_OneLiners");
    m->confidence = 0.95f;
    m->matched = 1;
    return 1;
  }
  if (contains_ci(text, "Runtime.getRuntime().exec(") && contains_ci(text, "request.getParameter")) {
    snprintf(m->rule_name, sizeof(m->rule_name), "%s", "JSP_Webshell_Runtime_Exec");
    m->confidence = 0.90f;
    m->matched = 1;
    return 1;
  }
  if ((contains_ci(text, "Process.Start(") || contains_ci(text, "new Process()")) && contains_ci(text, "Request.")) {
    snprintf(m->rule_name, sizeof(m->rule_name), "%s", "ASPX_Webshell_CommandExec");
    m->confidence = 0.85f;
    m->matched = 1;
    return 1;
  }
  return 0;
}

static int scan_webshell(const char *path, WebshellRuleMatch *out) {
  memset(out, 0, sizeof(*out));
  float ave_conf = 0.0f;
  AVEScanResult avr;
  memset(&avr, 0, sizeof(avr));
  if (AVE_ScanFile(path, &avr) == AVE_OK && avr.final_confidence > 0.0f) {
    ave_conf = avr.final_confidence;
  }
#ifdef EDR_HAVE_YARA
  if (s_yara_rules) {
    WebshellRuleMatch m;
    memset(&m, 0, sizeof(m));
    if (yr_rules_scan_file(s_yara_rules, path, 0, yara_scan_cb, &m, 0) == ERROR_SUCCESS && m.matched) {
      float c = m.confidence * 0.7f + ave_conf * 0.3f;
      if (ave_conf > c) {
        c = ave_conf;
      }
      m.confidence = c;
      *out = m;
      return 1;
    }
  }
#endif
  char *txt = NULL;
  if (read_file_text(path, &txt) != 0) {
    return 0;
  }
  WebshellRuleMatch m;
  memset(&m, 0, sizeof(m));
  int hit = fallback_match(txt, &m);
  free(txt);
  if (!hit) {
    return 0;
  }
  {
    float c = m.confidence * 0.7f + ave_conf * 0.3f;
    if (ave_conf > c) {
      c = ave_conf;
    }
    m.confidence = c;
    *out = m;
  }
  return 1;
}

static void push_alert(const char *file_path, const char *action, const WebRoot *root, const WebshellRuleMatch *m) {
  if (!s_bus || !file_path || !root || !m) {
    return;
  }
  char url[512];
  char alert_id[64];
  char fp[32];
  char object_key[512];
  char staged_path[1024];
  int file_uploaded = 0;
  edr_webshell_make_alert_id(alert_id, sizeof(alert_id));
  if (edr_webshell_file_fingerprint(file_path, fp, sizeof(fp)) != 0) {
    fp[0] = '\0';
  }
  object_key[0] = '\0';
  staged_path[0] = '\0';
  {
    struct _stat64 st;
    uint64_t up_max = (uint64_t)s_cfg->webshell_detector.max_upload_size_mb * 1024ULL * 1024ULL;
    if (s_cfg->webshell_detector.upload_webshell_files && _stat64(file_path, &st) == 0 && (uint64_t)st.st_size <= up_max) {
      const char *tenant = "tenant_default";
      if (s_cfg->agent.tenant_id[0]) {
        tenant = s_cfg->agent.tenant_id;
      }
      if ((edr_ingest_http_upload_file_multipart(alert_id, file_path, fp[0] ? fp : "", object_key, sizeof(object_key)) == 0 ||
           edr_grpc_client_upload_file(alert_id, file_path, fp[0] ? fp : "", object_key, sizeof(object_key)) == 0) &&
          object_key[0]) {
        file_uploaded = 1;
      } else {
        const char *root_dir =
            s_cfg->shellcode_detector.forensic_dir[0] ? s_cfg->shellcode_detector.forensic_dir : ".\\edr_forensic";
        file_uploaded = edr_webshell_stage_file(file_path, root_dir, tenant, alert_id, object_key, sizeof(object_key),
                                                staged_path, sizeof(staged_path));
      }
    }
  }
  infer_web_url(root, file_path, url, sizeof(url));
  EdrEventSlot slot;
  memset(&slot, 0, sizeof(slot));
  slot.timestamp_ns = now_ns();
  slot.type = EDR_EVENT_WEBSHELL_DETECTED;
  slot.priority = (m->confidence >= s_cfg->webshell_detector.l2_review_threshold) ? 0 : 1;
  slot.consumed = false;
  int n = snprintf((char *)slot.data, EDR_MAX_EVENT_PAYLOAD,
                   "ETW1\nprov=webshell\ndetector=%s\nrule=%s\nscore=%.6f\nfile=%s\nscript=service=%s action=%s "
                   "url=%s alert_id=%s file_fp=%s file_uploaded=%d object_key=%s local_path=%s ast_score=%.3f "
                   "token_score=%.3f\n",
                   strncmp(m->rule_name, "WebShell_AST_Token_", 19u) == 0 ? "semantic" : "yara", m->rule_name,
                   m->confidence, file_path, root->service_name, action ? action : "-", url, alert_id,
                   fp[0] ? fp : "-", file_uploaded, object_key[0] ? object_key : "-",
                   staged_path[0] ? staged_path : "-", m->ast_score, m->token_score);
  if (n < 0 || (size_t)n >= EDR_MAX_EVENT_PAYLOAD) {
    return;
  }
  slot.size = (uint32_t)n;
  (void)edr_event_bus_try_push(s_bus, &slot);
}

static void handle_change(const char *full_path, const char *action) {
  const WebRoot *root = find_root_for_path(full_path);
  if (!root || !pre_filter(full_path)) {
    return;
  }
  WebshellRuleMatch m;
  if (!scan_webshell(full_path, &m)) {
    return;
  }
  if (m.confidence < (float)s_cfg->webshell_detector.alert_threshold) {
    return;
  }
  push_alert(full_path, action, root, &m);
  EDR_LOGV("[webshell_detector] alert %.2f rule=%s file=%s\n", m.confidence, m.rule_name, full_path);
}

static DWORD WINAPI watch_thread_main(LPVOID param) {
  WatchEntry *entry = (WatchEntry *)param;
  if (!entry) {
    return 0;
  }
  uint8_t buf[65536];
  while (InterlockedCompareExchange(&s_stop, 0, 0) == 0 && entry->active) {
    DWORD bytes = 0;
    BOOL ok = ReadDirectoryChangesW(entry->dir_handle, buf, (DWORD)sizeof(buf), s_cfg->webshell_detector.monitor_subdirs,
                                    FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_LAST_WRITE |
                                        FILE_NOTIFY_CHANGE_CREATION,
                                    &bytes, NULL, NULL);
    if (!ok || bytes == 0u) {
      if (InterlockedCompareExchange(&s_stop, 0, 0) != 0) {
        break;
      }
      Sleep(100u);
      continue;
    }
    FILE_NOTIFY_INFORMATION *fni = (FILE_NOTIFY_INFORMATION *)buf;
    for (;;) {
      int wlen = (int)(fni->FileNameLength / sizeof(WCHAR));
      if (wlen > 0) {
        char name_utf8[MAX_PATH];
        int n = WideCharToMultiByte(CP_UTF8, 0, fni->FileName, wlen, name_utf8, (int)sizeof(name_utf8) - 1, NULL, NULL);
        if (n > 0) {
          name_utf8[n] = '\0';
          char full[MAX_PATH];
          snprintf(full, sizeof(full), "%s\\%s", entry->root_path, name_utf8);
          if (fni->Action == FILE_ACTION_ADDED) {
            handle_change(full, "created");
          } else if (fni->Action == FILE_ACTION_MODIFIED) {
            handle_change(full, "modified");
          } else if (fni->Action == FILE_ACTION_RENAMED_NEW_NAME) {
            handle_change(full, "moved_in");
          }
        }
      }
      if (fni->NextEntryOffset == 0u) {
        break;
      }
      fni = (FILE_NOTIFY_INFORMATION *)((uint8_t *)fni + fni->NextEntryOffset);
    }
  }
  return 0;
}

EdrError edr_webshell_detector_init(const EdrConfig *cfg, EdrEventBus *bus) {
  if (!cfg) {
    return EDR_ERR_INVALID_ARG;
  }
  if (!cfg->webshell_detector.enabled) {
    return EDR_OK;
  }
  if (s_started) {
    return EDR_OK;
  }
  s_cfg = cfg;
  s_bus = bus;
  discover_web_roots();
  if (s_root_count == 0u) {
    EDR_LOGE("%s", "[webshell_detector] no web roots discovered; set EDR_WEBSHELL_ROOTS to enable\n");
    return EDR_OK;
  }
#ifdef EDR_HAVE_YARA
  if (load_yara(cfg->webshell_detector.webshell_rules_dir) < 0) {
    EDR_LOGV("%s", "[webshell_detector] yara unavailable, fallback to builtin rules\n");
  }
#endif
  InterlockedExchange(&s_stop, 0);
  s_watch_count = 0;
  for (size_t i = 0; i < s_root_count && s_watch_count < cfg->webshell_detector.max_watch_dirs &&
                     s_watch_count < WEBSHELL_MAX_WATCHES;
       i++) {
    HANDLE h = CreateFileA(s_roots[i].root, FILE_LIST_DIRECTORY,
                           FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING,
                           FILE_FLAG_BACKUP_SEMANTICS, NULL);
    if (h == INVALID_HANDLE_VALUE) {
      continue;
    }
    WatchEntry *we = &s_watches[s_watch_count];
    memset(we, 0, sizeof(*we));
    we->dir_handle = h;
    we->active = 1;
    snprintf(we->root_path, sizeof(we->root_path), "%s", s_roots[i].root);
    we->thread = CreateThread(NULL, 0, watch_thread_main, we, 0, NULL);
    if (!we->thread) {
      we->active = 0;
      CloseHandle(we->dir_handle);
      we->dir_handle = INVALID_HANDLE_VALUE;
      continue;
    }
    s_watch_count++;
  }
  if (s_watch_count == 0u) {
    return EDR_OK;
  }
  s_started = 1;
  EDR_LOGV("[webshell_detector] Windows watcher started roots=%zu watches=%zu\n", s_root_count, s_watch_count);
  return EDR_OK;
}

void edr_webshell_detector_shutdown(void) {
  if (!s_started) {
    return;
  }
  InterlockedExchange(&s_stop, 1);
  for (size_t i = 0; i < s_watch_count; i++) {
    s_watches[i].active = 0;
    if (s_watches[i].dir_handle && s_watches[i].dir_handle != INVALID_HANDLE_VALUE) {
      CancelIoEx(s_watches[i].dir_handle, NULL);
      CloseHandle(s_watches[i].dir_handle);
      s_watches[i].dir_handle = INVALID_HANDLE_VALUE;
    }
  }
  for (size_t i = 0; i < s_watch_count; i++) {
    if (s_watches[i].thread) {
      WaitForSingleObject(s_watches[i].thread, 5000u);
      CloseHandle(s_watches[i].thread);
      s_watches[i].thread = NULL;
    }
  }
  s_watch_count = 0;
  s_root_count = 0;
  s_started = 0;
  s_cfg = NULL;
  s_bus = NULL;
#ifdef EDR_HAVE_YARA
  unload_yara();
#endif
}

unsigned int edr_webshell_detector_watch_count(void) {
  return (unsigned int)s_watch_count;
}

uint64_t edr_webshell_detector_budget_drop_count(void) { return s_budget_drops; }
