...
static int set_signing_public_key(const char *pem) {
#ifdef _WIN32
  return _putenv_s("EDR_COMMAND_SIGNING_PUBLIC_KEY", pem);
#else
  return setenv("EDR_COMMAND_SIGNING_PUBLIC_KEY", pem, 1);
#endif
}
