...
int main(void) {
  ...
  ok &= require_contains(collector, "edr_collector_self_identity_matches(&s_agent_self_identity",
                         "self-noise suppression must compare the exact live Agent generation");
  ok &= require_contains(collector, "edr_process_generation_query_live(GetCurrentProcess()",
                         "self identity must originate from the current process handle");
  ok &= require_absent(collector, "s_agent_self_pid_cache",
                       "event text and ancestry must not populate a descendant exemption cache");
  ...
  ok &= require_range_absent(collector, "static void edr_agent_self_count_drop_source",
                             "static int edr_policy_canary_marker", "s_health.collector_dropped++",
