...
if (Test-Path -LiteralPath $out) { throw 'USB input destination must not already exist' }
...
foreach ($name in @('scripts','install','config','rules','third_party\windivert','src\shellcode_detector\rules','src\webshell_detector\rules','VERSION','agent.toml.example')) {
    ...
    if (-not (Test-Path -LiteralPath $path)) { throw "USB input source is missing: $name" }
...
if ($LASTEXITCODE -ne 0 -or $sha -cnotmatch '^[a-f0-9]{40}$') { throw 'Cannot bind USB signing input to its source commit' }
$inventory = @(
...
[ordered]@{ schema='edr.windows.usb-signing-input.v1'; version=$Version; architecture=$Architecture; source_commit=$sha; files=$inventory } |
    ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $out 'signing-input.json') -Encoding UTF8
Write-Host "USB signing input staged: $Architecture $Version source=$sha files=$($inventory.Count)"
