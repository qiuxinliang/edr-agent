...
- **进程内 API**：`include/edr/ave_sdk.h`（`AVE_ScanFile`、`AVE_GetStatus` 等）。
...
| **模型/训练运维** | 终端不消费模型、模型热更新或联邦训练数据；平台模型运维与终端 `AVE_GetStatus` 无数据链关系。 |
...
| **管理端模型页面** | 与终端规则/IOC/行为启发式执行无直接数据链。 |
...
- **建议**：在 **ingest 实现侧** 持久化 `command_type` + `detail_utf8`（或结构化 JSON），并扩展 **`response-tasks` 或新资源**（如 `command-results`）供 **`edr-frontend`** 列表与详情展示；终端 `AVE_GetStatus` 以规则、IOC、抑制和行为队列状态为展示基线。
...
| **规则与发布** | 平台发布 Agent 版本及签名策略；规则、IOC 和租户策略通过现有可信配置链管理，不下发或热更新端侧模型。 |
| **可选增强** | 周期性或在 heartbeat/库存扩展中上报 **`AVE_GetStatus`** 风格字段（规则版本、IOC 版本、队列与抑制统计）需 **ingest / 库存 API** 与前端列对齐；当前以 **指令 `ave_status`**（`edr_ave_get_scan_counts` 摘要）为基线联调路径。 |
...
| **`ave_infer` 展示** | 成功时解析或展示 **`detail_utf8`** 中的 `final=`、`sha256=`、`dur_ms=` 等（具体格式以 `command_stub.c` / SDK 实现为准）；失败时展示 **`exit_code`** 与 **`detail_utf8`**，用于区分策略拒绝、路径错误和执行错误。 |
| **`ave_status` 展示** | 将规则/IOC 版本、队列、抑制和行为统计以只读面板展示即可（来源为指令回包 `detail`）。 |
| **设计稿交叉引用** | 终端列表中的 AVE 信息应展示规则与运行时健康状态，不应展示端侧模型版本。参见 **`Cauld Design/EDR_前端详细设计_v1.0.md`** 中与终端详情、批量操作、狩猎相关的章节，随 **edr-backend** 接口落地迭代。 |

---

## 5. 联调验证（终端仓库）

在 **`edr-agent`** 完成标准产品构建后：

```bash
ctest -R 'ave_' --output-on-failure
```

应包含 AVE 规则、IOC、误报抑制、行为特征与单元测试等（以 **`CMakeLists.txt`** 中 `add_test(NAME ave_* ...)` 为准）。完整边界见 **`docs/AVE_ENGINE_IMPLEMENTATION_PLAN.md`**。
