...
| **P0-3** | Windows 默认 `model_dir` | 默认仍为 Unix 风格路径，易误用 | **已做**：`config.c` 在 **`#ifdef _WIN32`** 下默认 `C:\\ProgramData\\EDR\\models`；`agent.toml.example` 注释补充 Windows 示例 | 无 `[ave]` 配置时 Win 默认路径与文档一致 |
