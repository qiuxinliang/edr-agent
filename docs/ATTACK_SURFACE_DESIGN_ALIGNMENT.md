...
**权威代码路径**：`src/core/agent.c`（`edr_agent_poll_attack_surface` 兼容轮询与周期调度）、`src/attack_surface/attack_surface_report.c`（`edr_attack_surface_execute`、`edr_attack_surface_effective_periodic_interval_s`、出站/策略**并行采集**、`snapshotKind` **full / listenersOnly**、内置 ingest HTTP POST、ETW 去抖）、`src/command/command_stub.c`（`GET_ATTACK_SURFACE` / `REFRESH_ATTACK_SURFACE` 手动命令）。gRPC 演进见 **`docs/ATTACK_SURFACE_GRPC.md`**。

---

## 0. 产品策略（已确认）

1. **上报通道**：生产环境以 **REST JSON（内置 ingest HTTP POST）** 为**唯一权威**；是否走 HTTP/2 由系统组选定的传输配置决定，详设 §19.5 的 **gRPC `ReportSnapshot`** 保留为**后续可选**能力（见 **`ATTACK_SURFACE_GRPC.md`**），不默认双写。
...
| 手动 **`GET_ATTACK_SURFACE` / `REFRESH_ATTACK_SURFACE`** | 平台 HTTP command outbox 下发命令后进入 **`edr_attack_surface_execute`**，**不检查 `enabled`** | 与周期开关解耦（见 `README`）；前端按钮不依赖 `[attack_surface].enabled=true` |
...
| 详设 §19.5 | 当前实现 | 备注 |
|------------|----------|------|
| gRPC **`AttackSurfaceService.ReportSnapshot`** | **HTTP `POST`** `{rest_base}/endpoints/{endpoint_id}/attack-surface`，`Content-Type: application/json`，通过 **`edr_ingest_http_post_json_suffix`** 复用 REST base、认证、TLS/proxy 与系统组选定的 HTTP/HTTP2/fallback 传输配置 | 与 Proto 示例不一致；**以 REST 为端点实现准** |
| **`RequestSnapshot`**（服务端 → Agent） | 控制台主路径：后端 `POST .../attack-surface/refresh` 创建 response task 并通过 HTTP command outbox 下发 **`REFRESH_ATTACK_SURFACE`**；旧 **GET `refresh-request`** + 轮询仅保留兼容 | 产品流程可等价，**协议不同** |
...
- **已实现**：周期全量（按四间隔最小值）、HTTP command outbox 手动 **`REFRESH_ATTACK_SURFACE`**、refresh-request 兼容轮询、ETW 去抖触发、（条件）启动快照、按需 POST；可选配置重载后 POST；上报 **REST JSON（内置 ingest HTTP，按系统传输配置选择 HTTP/HTTP2/fallback）** 稳定可用。
